...
//    }
    ...
    // Suppose we have a denoised spectrum
    kiss_fft_cpx denoised[FRAME_SIZE] = {};

    // Find magnitude of denoised spectrum
    float denoised_magnitude[FRAME_SIZE] = {};

    for (int i=0; i<FRAME_SIZE; i++) {
        float temp = (denoised[i].r*denoised[i].r) + (denoised[i].i*denoised[i].i);
        denoised_magnitude[i] = sqrt(temp);
    }

    float hps[FRAME_SIZE] = {};

    for (int i=0; i<FRAME_SIZE; i++) {
        float prod = denoised_magnitude[i];
        if ((2*i < FRAME_SIZE) && (denoised_magnitude[2*i] > 0)) {
            prod *= denoised_magnitude[2*i];
        }
        if ((4*i < FRAME_SIZE) && (denoised_magnitude[4*i] > 0)) {
            prod *= denoised_magnitude[4*i];
        }
        if ((8*i < FRAME_SIZE) && (denoised_magnitude[8*i] > 0)) {
